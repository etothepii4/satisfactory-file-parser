...
export namespace SaveBodyChunks {
	...
	export const DecompressChunks = (fileBuffer: ArrayBufferLike, alignment = Alignment.LITTLE_ENDIAN): SaveBodyChunks => {
		...
		do {
			...
			packageFileTag = array.getUint32(0, alignment === Alignment.LITTLE_ENDIAN);
			const headerVersion = array.getUint32(4, alignment === Alignment.LITTLE_ENDIAN);
			chunkHeaderSize = headerVersion === HEADER_V1 ? 48 : 49;
			maxUncompressedChunkContentSize = array.getInt32(8, alignment === Alignment.LITTLE_ENDIAN);
			if (headerVersion === HEADER_V2) {
				compressionAlgorithm = array.getUint8(16);
			}
			const chunkUncompressedLength = array.getInt32(headerVersion === HEADER_V1 ? 24 : 25, alignment === Alignment.LITTLE_ENDIAN);
			const chunkCompressedLength = array.getInt32(headerVersion === HEADER_V1 ? 32 : 33, alignment === Alignment.LITTLE_ENDIAN);
			...
			try {
				...
				currentInflatedChunk = Pako.inflate(fileBuffer.slice(currentByte, currentByte + chunkCompressedLength));
				currentChunks.push(currentInflatedChunk);
				
				// step to next chunk
				currentByte += chunkCompressedLength;
				array = new DataView(fileBuffer.slice(currentByte));
